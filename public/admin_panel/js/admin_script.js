...
$(document).ready(function($){
  $('.bill_cancel').click(function(){
      ...
      console.log(check_bill_status);
      if (check_bill_status == 'Đang chờ') {
        var r = confirm("Bạn chắc chắn muốn hủy đơn hàng ?");
        if (r == true) {
            ...
            var success = function(){
                $('#bill_cancel_id').html('Bạn vừa hủy bỏ thành công đơn hàng số: ' + id);
                $('#bill'+id).remove();
                $('#biil_destroy_success').modal('show');
            }
            var dataType = 'text';
            $.get(url,data,success,dataType);
        } else {
